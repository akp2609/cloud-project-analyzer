...
const requestDuration = new client.Histogram({
    ...
    labelNames: ["method","route","status","tenant", "hookId"],
...
const metricsMiddleware = (req,res,next)=>{
    ...
    const hook_id = req.headers["x-hook-id"];

    onHeaders(res,()=>{
        const duration = Date.now() - start;
        const status = String(res.statusCode);
        requestDuration.labels(req.method,route,status,tenant,hook_id).observe(duration);
        requestCount.labels(req.method,route,status,tenant,hook_id).inc();
    });
